...
export default function TodoOverview({ item, lang }: TodoOverviewProps) {
  return (
    <div
      className="shadow ml-2 mr-2 dark:border"
      style={{ opacity: item.completed ? "0.2" : "1" }}
    >
      <div
        className={
          "h-1 w-full rounded-t " + levelColor.get(item.level.toString())
        }
      ></div>
        ...
        <div className="flex items-center justify-between space-x-2 ml-2 pr-4 w-full">
          <div className="flex items-center space-x-1">
            ...
            <p className="font-semibold text-xs">
              {item.completed
                ? lang["todooverview_completed"]
                : lang["todooverview_uncompleted"]}
            </p>
          </div>
          <div className="flex items-center space-x-1">
            <BellIcon />{" "}
